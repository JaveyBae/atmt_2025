...
def decode(model: Seq2SeqModel, src_tokens: torch.Tensor, src_pad_mask: torch.Tensor, max_out_len: int,
    ...
    # Optimization: Run encoder once before the loop
    with torch.no_grad():
    ...
    for t in range(max_out_len):
        ...
        # Optimization: Use decoder directly with pre-computed memory
        with torch.no_grad():
            output = model.decoder(encoder_output, src_pad_mask, generated, trg_pad_mask).to(device)
    ...
    for _ in range(max_out_len):
        ...
        for seq, score in beams:
            ...
            with torch.no_grad():
                ...
                # Optimization: Use decoder directly with pre-computed memory
                output = model.decoder(encoder_output, src_pad_mask, seq, trg_pad_mask)
                logits = output[:, -1, :]
